...
-  Quick sort picks a random element and partition the array, such that all the numbers that are less than the partitioning element come before all elements that are greater than it
  -  the partinioning can be performed efficiently through a series of swaps

When to use:
-  on smaller datasets (doesn't work well with larger)
-  preferred for using on arrays
